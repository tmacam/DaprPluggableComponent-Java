...
RUN  apk update \
  && apk upgrade \
  && apk add --no-cache --update tzdata wget unzip bash \
  && rm -rf /var/cache/apk/*
...
#  Install Gradle
#
# We install gradle directly from its sources as its apk package
# pulls way too many unecessary dependencies (+300MB of them)
...
# Glibc compatibility and protobuf runtime libs
RUN apk update \
    && apk upgrade \
    && apk add --no-cache --update gcompat protobuf \
    && rm -rf /var/cache/apk/*
RUN mkdir -vp /opt/
ENV PROJECT_DIR=/opt/statestore-component
