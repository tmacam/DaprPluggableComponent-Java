...
public interface LockStore extends InitializableWithProperties {
  ...
  boolean tryLock(String requestLockOwner, String requestResourceId, int requestExpiryInSeconds);

  UnlockStatus unlock(String requestLockOwner, String requestResourceId);

  enum UnlockStatus {
    Success(0), LockDoesNotExist(1), LockBelongsToOther(2), InternalError(3);
